...
// to defend against shape-id reuse across rebuilds (each build reset
// shapeId to 0, so a stale prewarm could poison snapCache with distances
// under IDs the new index had reused). With shapeIdCounter now globally
// unique, that hazard is gone — and cancelling actively hurts: a user
// who flips subway → LIRR before subway's prewarm runs would lose
// subway's prewarm permanently, since the WeakMap-cached subway index
// returns instantly on re-entry without rescheduling.
